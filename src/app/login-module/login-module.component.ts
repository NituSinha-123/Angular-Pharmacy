import { Component, EventEmitter, getModuleFactory, OnInit } from '@angular/core';
...
export class LoginModuleComponent implements OnInit {
  ...
  ngOnInit(): void {
   // this.userid=this.userDtls.userid
  ...
  onLogin(custForm:NgForm)
  {
    ...
    this.userid=custForm.value.userid;
    ...
    this.userService.loginMember(this.user).subscribe(data => {
      ...
      response.subscribe(data1=>{
        ...
        if(this.authService.isLoggedIn()){
         ...
         // this.router.navigate(['subscriptions'],{state:{data:this.userid}});
         //this.router.navigate(["subscriptions/"+this.userid]);
        }
